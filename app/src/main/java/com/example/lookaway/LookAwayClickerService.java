...
public class LookAwayClickerService extends AccessibilityService {
    ...
    private List<String> trapClasses;
    ...
    private final BroadcastReceiver trapControlReceiver = new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {
            if ("com.example.lookaway.RELOAD_TRAP_CLASSES".equals(intent.getAction())) {
                trapClasses = LookAwayConfigLoader.getList(LookAwayClickerService.this, "class");
            }
        }
    };
    ...
    protected void onServiceConnected() {
        ...
        instance = this;
        trapClasses = LookAwayConfigLoader.getList(this, "class");

        IntentFilter filter = new IntentFilter("com.example.lookaway.RELOAD_TRAP_CLASSES");
        ContextCompat.registerReceiver(this, trapControlReceiver, filter, ContextCompat.RECEIVER_NOT_EXPORTED);
        ...
        if (resolveInfo != null && resolveInfo.activityInfo != null) homeLauncherPackage = resolveInfo.activityInfo.packageName;
    ...
    public void onAccessibilityEvent(AccessibilityEvent event) {
        if (event.getEventType() == AccessibilityEvent.TYPE_WINDOW_STATE_CHANGED) {
            ...
            // 2. Scan Logic
            boolean isKnownAdClass = false;
            String lowerClass = currentClass.toLowerCase();
            for (String trap : trapClasses) {
                if (lowerClass.contains(trap)) {
                    isKnownAdClass = true;
                    break;
                }
            }
    ...
    public void onDestroy() {
        super.onDestroy();
        try { unregisterReceiver(trapControlReceiver); } catch (Exception ignored) {}
        instance = null;
    }
}